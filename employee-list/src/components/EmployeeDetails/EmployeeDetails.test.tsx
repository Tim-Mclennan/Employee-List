...
mockAxios.get.mockImplementationOnce(() => Promise.resolve({data: 'mocked response'}));
...
describe('EmployeeDetails component', () => {
  it('should display the employee details form', async () => {
    ...
    expect(getByLabelText('Middle Name: (if applicable)')).toHaveAttribute('value', 'Doe');
    expect(getByLabelText('Last Name:')).toHaveAttribute('value', 'Smith');
    expect(getByLabelText('Email:')).toHaveAttribute('value', 'johndoe@example.com');
    expect(getByLabelText('Mobile Number:')).toHaveAttribute('value', '+61412345678');
    expect(getByLabelText('Residential Address:')).toHaveAttribute('value', '1 Example St, Sydney');
  ...
  it('should display an error message if the request fails', async () => {
    ...
    it('renders the error message when a PUT request returns a 401 error', async () => {
        ...
        const errorMessage = getByText('Unauthorized: You need to be logged in to perform this action.');
      ...
      it('renders the error message when a PUT request returns a 403 error', async () => {
        ...
        const errorMessage = getByText('Forbidden: You do not have the necessary permissions to perform this action.');
        expect(errorMessage).toBeInTheDocument();
      });
      ...
      it('renders the error message when a PUT request returns a 404 error', async () => {
        ...
        const errorMessage = getByText('Not Found: The requested resource could not be found on the server.');
        expect(errorMessage).toBeInTheDocument();
      });
